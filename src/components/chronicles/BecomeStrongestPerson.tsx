...
import img1 from "./be_strong.png";
import img2 from "./desert.png";
...
const StrongestPerson = () => {
  const { t } = useTranslation("articles");
  const articleTexts: ArticleType[] = t("article12", { ns: "articles", returnObjects: true });
  ...
  return (
    <div>
      ...
      <div className="article-container">
        {articleTexts.map((article12, index) => (
          <div key={index} className="art-card">
            <h4 className="art-title">{article12.title}</h4>
            <div className="authors-box">
              <span className="date-and-duration">{article12.date} &#183; {article12.duration}</span>
              <span className="authors">{t("authors")}</span>
            </div>
            <div className="responder-box">
              <div className="resp-left">
                <img className="resp-image" src={article12.image} alt={article12.image} />
                <div className="resp-name">{article12.responder}</div>
                <div className="resp-role">{article12.role}</div>
              </div>
            ...
            <div className="art-detail">
              {article12.content.map((item, index) => (
                <div key={index}>
...
export default StrongestPerson;