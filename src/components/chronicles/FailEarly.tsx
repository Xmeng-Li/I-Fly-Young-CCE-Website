...
const FailEarly = () => {
  ...
  const articleTexts: ArticleType[] = t("article14", { ns: "articles", returnObjects: true });
  ...
  return (
    <div>
      ...
      <div className="article-container">
        {articleTexts.map((article14, index) => (
          <div key={index} className="art-card">
            <h4 className="art-title">{article14.title}</h4>
            <div className="authors-box">
              <span className="date-and-duration">{article14.date} &#183; {article14.duration}</span>
              <span className="authors">{t("authors")}</span>
            </div>
            <div className="responder-box">
              <div className="resp-left">
                <img className="resp-image" src={article14.image} alt={article14.image} />
                <div className="resp-name">{article14.responder}</div>
                <div className="resp-role">{article14.role}</div>
              </div>
            ...
            <div className="art-detail">
              {article14.content.map((item, index) => (
                <div key={index}>
