...
const ExploreRealWorld = () => {
  const { t } = useTranslation("articles");
  const articleTexts: ArticleType[] = t("article4", { ns: "articles", returnObjects: true });
  ...
  return (
    <div>
      ...
      <div className="article-container">
        {articleTexts.map((article4, index) => (
          <div key={index} className="art-card">
            <h4 className="art-title">{article4.title}</h4>
            <div className="authors-box">
              <span className="date-and-duration">{article4.date} &#183; {article4.duration}</span>
              <span className="authors">{t("authors")}</span>
            </div>
            <div className="responder-box">
              <div className="resp-left">
                <img className="resp-image" src={article4.image} alt={article4.image} />
                <div className="resp-name">{article4.responder}</div>
                <div className="resp-role">{article4.role}</div>
              </div>
            ...
            <div className="art-detail">
              {article4.content.map((item, index) => (
                <div key={index}>
