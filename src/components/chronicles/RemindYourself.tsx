...
const RemindYourself = () => {
  const { t } = useTranslation("articles");
  const articleTexts: ArticleType[] = t("article13", { ns: "articles", returnObjects: true });
  ...
  return (
    <div>
      ...
      <div className="article-container">
        {articleTexts.map((article13, index) => (
          <div key={index} className="art-card">
            <h4 className="art-title">{article13.title}</h4>
            <div className="authors-box">
              <span className="date-and-duration">{article13.date} &#183; {article13.duration}</span>
              <span className="authors">{t("authors")}</span>
            </div>
            <div className="responder-box">
              <div className="resp-left">
                <img className="resp-image" src={article13.image} alt={article13.image} />
                <div className="resp-name">{article13.responder}</div>
                <div className="resp-role">{article13.role}</div>
              </div>
            ...
            <div className="art-detail">
              {article13.content.map((item, index) => (
                <div key={index}>
