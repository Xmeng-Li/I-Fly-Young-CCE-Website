...
import img1 from "./drawing.png";
import img2 from "./drawing_lady.png";
...
const DoSomethingYouLike = () => {
  const { t } = useTranslation("articles");
  const articleTexts: ArticleType[] = t("article8", { ns: "articles", returnObjects: true });
  ...
  return (
    <div>
      ...
      <div className="article-container">
        {articleTexts.map((article8, index) => (
          <div key={index} className="art-card">
            <h4 className="art-title">{article8.title}</h4>
            <div className="authors-box">
              <span className="date-and-duration">{article8.date} &#183; {article8.duration}</span>
              <span className="authors">{t("authors")}</span>
            </div>
            <div className="responder-box">
              <div className="resp-left">
                <img className="resp-image" src={article8.image} alt={article8.image} />
                <div className="resp-name">{article8.responder}</div>
                <div className="resp-role">{article8.role}</div>
              </div>
            ...
            <div className="art-detail">
              {article8.content.map((item, index) => (
                <div key={index}>
