...
const CertificationHelps = () => {
  const { t } = useTranslation("articles");
  const articleTexts: ArticleType[] = t("article10", { ns: "articles", returnObjects: true });
  ...
  return (
    <div>
      ...
      <div className="article-container">
        {articleTexts.map((article10, index) => (
          <div key={index} className="art-card">
            <h4 className="art-title">{article10.title}</h4>
            <div className="authors-box">
              <span className="date-and-duration">{article10.date} &#183; {article10.duration}</span>
              <span className="authors">{t("authors")}</span>
            </div>
            <div className="responder-box">
              <div className="resp-left">
                <img className="resp-image" src={article10.image} alt={article10.image} />
                <div className="resp-name">{article10.responder}</div>
                <div className="resp-role">{article10.role}</div>
              </div>
            ...
            <div className="art-detail">
              {article10.content.map((item, index) => (
                <div key={index}>
