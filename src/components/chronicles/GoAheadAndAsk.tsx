...
const GoAheadAndAsk = () => {
  ...
  const articleTexts: ArticleType[] = t("article15", { ns: "articles", returnObjects: true });
  ...
  return (
    <div>
      ...
      <div className="article-container">
        {articleTexts.map((article15, index) => (
          <div key={index} className="art-card">
            <h4 className="art-title">{article15.title}</h4>
            <div className="authors-box">
              <span className="date-and-duration">{article15.date} &#183; {article15.duration}</span>
              <span className="authors">{t("authors")}</span>
            </div>
            <div className="responder-box">
              <div className="resp-left">
                <img className="resp-image" src={article15.image} alt={article15.image} />
                <div className="resp-name">{article15.responder}</div>
                <div className="resp-role">{article15.role}</div>
              </div>
            ...
            <div className="art-detail">
              {article15.content.map((item, index) => (
                <div key={index}>
