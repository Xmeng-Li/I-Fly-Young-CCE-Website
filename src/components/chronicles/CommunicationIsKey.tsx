...
import img1 from "./talking_girl.png";
import img2 from "./talking_guy.png";
...
const CommunicationIsKey = () => {
  const { t } = useTranslation("articles");
  const articleTexts: ArticleType[] = t("article11", { ns: "articles", returnObjects: true });
  ...
  return (
    <div>
      ...
      <div className="article-container">
        {articleTexts.map((article11, index) => (
          <div key={index} className="art-card">
            <h4 className="art-title">{article11.title}</h4>
            <div className="authors-box">
              <span className="date-and-duration">{article11.date} &#183; {article11.duration}</span>
              <span className="authors">{t("authors")}</span>
            </div>
            <div className="responder-box">
              <div className="resp-left">
                <img className="resp-image" src={article11.image} alt={article11.image} />
                <div className="resp-name">{article11.responder}</div>
                <div className="resp-role">{article11.role}</div>
              </div>
            ...
            <div className="art-detail">
              {article11.content.map((item, index) => (
                <div key={index}>
