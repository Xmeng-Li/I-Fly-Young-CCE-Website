...
import img1 from "./compass.png";
import img2 from "./grass.png";
...
const ConstExposure = () => {
  ...
  const articleTexts: ArticleType[] = t("article2", { ns: "articles", returnObjects: true });
  ...
  return (
    <div>
      ...
      <div className="article-container">
        {articleTexts.map((article2, index) => (
          <div key={index} className="art-card">
            <h4 className="art-title">{article2.title}</h4>
            <div className="authors-box">
              <span className="date-and-duration">{article2.date} &#183; {article2.duration}</span>
              <span className="authors">{t("authors")}</span>
            </div>
            <div className="responder-box">
              <div className="resp-left">
                <img className="resp-square-image" src={article2.image} alt={article2.image} />
                <div className="resp-name">{article2.responder}</div>
                <div className="resp-role">{article2.role}</div>
              </div>
            ...
            <div className="art-detail">
              {article2.content.map((item, index) => (
                <div key={index}>
