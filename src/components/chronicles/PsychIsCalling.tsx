...
import img1 from "./inside_us.png";
...
const PsychIsCalling = () => {
  const { t } = useTranslation("articles");
  const articleTexts: ArticleType[] = t("article6", { ns: "articles", returnObjects: true });
  ...
  return (
    <div>
      ...
      <div className="article-container">
        {articleTexts.map((article6, index) => (
          <div key={index} className="art-card">
            <h4 className="art-title">{article6.title}</h4>
            <div className="authors-box">
              <span className="date-and-duration">{article6.date} &#183; {article6.duration}</span>
              <span className="authors">{t("authors")}</span>
            </div>
            <div className="responder-box">
              <div className="resp-left">
                <img className="resp-square-image" src={article6.image} alt={article6.image} />
                <div className="resp-name">{article6.responder}</div>
                <div className="resp-role">{article6.role}</div>
              </div>
            ...
            <div className="art-detail">
              {article6.content.map((item, index) => (
                <div key={index}>
