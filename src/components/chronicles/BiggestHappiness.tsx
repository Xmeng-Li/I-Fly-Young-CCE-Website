...
import img1 from "./jumping_man.png";
import img2 from "./mom_kid.png";
...
const BiggestHappiness = () => {
  const { t } = useTranslation("articles");
  const articleTexts: ArticleType[] = t("article9", { ns: "articles", returnObjects: true });
  ...
  return (
    <div>
      ...
      <div className="article-container">
        {articleTexts.map((article9, index) => (
          <div key={index} className="art-card">
            <h4 className="art-title">{article9.title}</h4>
            <div className="authors-box">
              <span className="date-and-duration">{article9.date} &#183; {article9.duration}</span>
              <span className="authors">{t("authors")}</span>
            </div>
            <div className="responder-box">
              <div className="resp-left">
                <img className="resp-image" src={article9.image} alt={article9.image} />
                <div className="resp-name">{article9.responder}</div>
                <div className="resp-role">{article9.role}</div>
              </div>
            ...
            <div className="art-detail">
              {article9.content.map((item, index) => (
                <div key={index}>
...
export default BiggestHappiness;