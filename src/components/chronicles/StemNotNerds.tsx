...
import img1 from "./tech_students.png";
import img2 from "./wires.png";
...
const StemNotNerds = () => {
  ...
  const articleTexts: ArticleType[] = t("article1", { ns: "articles", returnObjects: true });
  ...
  return (
    <div>
      ...
      <div className="article-container">
        {articleTexts.map((article1, index) => (
          <div key={index} className="art-card">
            <h4 className="art-title">{article1.title}</h4>
            <div className="authors-box">
              <span className="date-and-duration">{article1.date} &#183; {article1.duration}</span>
              <span className="authors">{t("authors")}</span>
            </div>
            <div className="responder-box">
              <div className="resp-left">
                <img className="resp-square-image" src={article1.image} alt={article1.image} />
                <div className="resp-name">{article1.responder}</div>
                <div className="resp-role">{article1.role}</div>
              </div>
            ...
            <div className="art-detail">
              {article1.content.map((item, index) => (
                <div key={index}>
