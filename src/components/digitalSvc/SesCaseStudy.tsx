...
import "../../styles/sesCase.css";

import topImg from "./SES-mockup.png";
import logo from "./ses_logo_white.png";
import home from "./ses-Home.png";
import curated from "./ses-Curated.png";
import curatedBook from "./ses-whereAre.png";
import print from "./ses-Print.png";
import bookstore from "./ses-Bookstore.png";
import bestseller from "./ses-Bestseller2.png";
...
const SesCase = () => {
  ...
  const caseContent: Content[] = t("contents", { ns: "ses", returnObjects: true });
  ...
  useEffect(() => {
    document.body.style.backgroundColor = "#F2F9E8";
    return () => {
  ...
  return (
    <div>
      <Header />
      <div className="sesCase-backTo">
        <Link to="/digital-service" className="left-arrow">
          <LeftArrow />
        </Link>
        <label className="sesCase-backTo-text">{t("backToSol")}</label>
      </div>
      <div className="sesCase-banner">
        <div className="sesCase-left-content">
          <span className="sesCase-sub">{t("caseSub")}</span>
          <span className="sesCase-title">{t("caseTitle")}</span>
          <img className="ses-logo" src={logo} alt="logo" />
        </div>
        <img className="ses-mockUp" src={topImg} alt="mockup" />
      </div>

      {/* Our Work */}
      <div className="ses-top-three">
        <label className="ses-work-label">{caseContent[0].works}</label>
        <label className="ses-work-sub">{caseContent[0].workSub}</label>
        <div className="ses-work-three">
          <div className="ses-work-part">
            <span className="ses-top-three-icons"><ThumbsIcon /></span>
            <div
              className="ses-work-text"
              dangerouslySetInnerHTML={{ __html: caseContent[0].leftText }}
            />
          </div>
          <div className="ses-work-part">
            <span className="ses-top-three-icons"><FaceIcon /></span>
            <div className="ses-work-text">{caseContent[0].midText}</div>
          </div>
          <div className="ses-work-part">
            <span className="ses-top-three-icons"><StarIcon /></span>
            <div
              className="ses-work-text"
      ...
      {/* Main */}
      <div className="ses-section-wrap">
        {/* Outcomes */}
        <div className="sec-detail">
          <div className="ses-outcomes-wrap">
            <label className="ses-work-label">
              {caseContent[1].secTitle1}
            </label>
            <ul className="ses-lst">
              {section?.impr1?.map((item, index) => (
                <li key={index}>{item}</li>
              ))}
              <p></p>
              <a href="https://samueleducation.com/" target="_blank" rel="noopener noreferrer">
                {t("viewSite")}
              </a> 
            </ul>
          </div>

          <div className="ses-comp-wrap">
            {/* Webpage */}
            <div className="ses-comp">
              <span className="ses-comp-label">{caseContent[1].home}</span>
              <img
                className="ses-site-img"
                src={home}
                alt="Home preview"
            ...
            <div className="ses-comp">
              <span className="ses-comp-label">{caseContent[1].curated}</span>
              <img
                className="ses-site-img"
                src={curated}
                alt="Curated preview"
                onClick={() => setPreviewImg(curated)}
              />
            </div>
          </div>
          <div className="ses-comp-wrap">
            {/* Webpage */}
            <div className="ses-comp">
              <span className="ses-comp-label">{caseContent[1].curatedSample}</span>
              <img
                className="ses-site-img"
                src={curatedBook}
                alt="Curated Sample preview"
                onClick={() => setPreviewImg(curatedBook)}
              />
            </div>
            <div className="ses-comp">
              <span className="ses-comp-label">{caseContent[1].print}</span>
              <img
                className="ses-site-img"
                src={print}
                alt="Print preview"
                onClick={() => setPreviewImg(print)}
              />
            </div>
          </div>
          <div className="ses-comp-wrap">
            {/* Webpage */}
            <div className="ses-comp">
              <span className="ses-comp-label">{caseContent[1].bookstore}</span>
              <img
                className="ses-site-img"
                src={bookstore}
                alt="Bookstore preview"
                onClick={() => setPreviewImg(bookstore)}
              />
            </div> 
            <div className="ses-comp">
              <span className="ses-comp-label">{caseContent[1].bestseller}</span>
              <img
                className="ses-site-img"
                src={bestseller}
                alt="Bestseller preview"
                onClick={() => setPreviewImg(bestseller)}
              />
            </div>
          </div>
        </div>
      ...
      {/* Bottom Part */}
      <div className="ses-btm">
        <div className="ses-btm-header">
          <label className="ses-btm-title">
            {t("svcBtmTitle")}
          </label>
          <span className="ses-btm-circle"><Circle /></span>
        </div>
        <div className="ses-btm-texts">{t("svcBtmTexts")}</div>
        <button className="ses-btm-btn">
      ...
      {previewImg && (
        <div className="ses-lightbox" onClick={() => setPreviewImg(null)}>
          <img
            className={`ses-lightbox-img ${zoomed ? "zoomed" : ""}`}
            src={previewImg}
            alt="Full preview"
            onClick={(e) => {
              e.stopPropagation();
